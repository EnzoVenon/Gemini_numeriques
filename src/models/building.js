...
// Menu de style
export function applyStyle(view, id, serverURL, version, nameType, crs, ipr, format, extent) {
    ...
    const layer = view.getLayerById(id);
    view.removeLayer(id);
    meshes = [];
    layer.delete();
    ...
    // Geometry Layer
    const geomLayer = new itowns.FeatureGeometryLayer(id, {
        batchId: function (property, featureId) { return featureId; },
        onMeshCreated: function scaleZ(mesh) {
            mesh.children.forEach(c => {
                c.scale.z = 0.01;
