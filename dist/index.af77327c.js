...
dat.GUI.prototype.colorLayerFolder = function colorLayerFolder(name, value) {
    ...
    if (title.style) title.style.background = value;
};
...
function GuiTools(domId, view, w) {
    if (view) {
        ...
        view.addEventListener("layers-order-changed", (function refreshColorGui() {
            var i;
            var colorLayers = view.getLayers(function filter(l) {
                return l.isColorLayer;
            });
            for(i = 0; i < colorLayers.length; i++)this.removeLayersGUI(colorLayers[i].id);
            this.addImageryLayersGUI(colorLayers);
        }).bind(this));
    }
}
GuiTools.prototype.addLayerGUI = function fnAddLayerGUI(layer) {
    if (layer.isColorLayer) this.addImageryLayerGUI(layer);
    else if (layer.isElevationLayer) this.addElevationLayerGUI(layer);
    else if (layer.isGeoidLayer) this.addGeoidLayerGUI(layer);
};
GuiTools.prototype.addLayersGUI = function fnAddLayersGUI() {
    function filterColor(l) {
        return l.isColorLayer;
    }
    function filterElevation(l) {
...
GuiTools.prototype.addImageryLayerGUI = function addImageryLayerGUI(layer) {
    ...
    }, "opacity", 0.0, 1.0, 0.01).onChange((function updateOpacity(value) {
        layer.opacity = value;
        this.view.notifyChange(layer);
    }).bind(this));
    folder.add({
        frozen: layer.frozen
    }, "frozen").onChange((function updateFrozen(value) {
        layer.frozen = value;
        this.view.notifyChange(layer);
    }).bind(this));
};
GuiTools.prototype.addElevationLayerGUI = function addElevationLayerGUI(layer) {
    ...
    folder.add({
        frozen: layer.frozen
    }, "frozen").onChange(function refreshFrozenGui(value) {
        layer.frozen = value;
    });
    folder.add({
        scale: layer.scale
    }, "scale").min(1.0).max(20000.0).onChange((function updateScale(value) {
        layer.scale = value;
        this.view.notifyChange(layer);
    }).bind(this));
};
...
GuiTools.prototype.addImageryLayersGUI = function addImageryLayersGUI(layers) {
    ...
    for(i = 0; i < sortedLayers.length; i++)this.addImageryLayerGUI(sortedLayers[i]);
};
GuiTools.prototype.addElevationLayersGUI = function addElevationLayersGUI(layers) {
    var i;
    for(i = 0; i < layers.length; i++)this.addElevationLayerGUI(layers[i]);
};
...
function createHTMLListFromObject(jsObject) {
    ...
    Object.keys(jsObject).forEach(function _(property) {
        ...
        if (jsObject[property] === null) jsObject[property] = "null";
        if (typeof jsObject[property] === "object") // if property value is an object, then recurse to
...
//# sourceMappingURL=index.af77327c.js.map
